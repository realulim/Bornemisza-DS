...
{% set DATA='{"type":"SRV","name":"_db._tcp.' + pillar['ssldomain'] + '.","content":"SRV 1 0 443 ' + pillar['hostname'] + '.","data":{"priority":1,"weight":0,"port":443,"target":"' + pillar['hostname'] + '","service":"_db","proto":"_tcp","name":"' + pillar['ssldomain'] + '","ttl":"1","proxied":false}}' %}

create-srv-record:
  cmd.run:
    - name: {{ CFCMD }} POST "{{ CFAPI }}/{{ CFZONEID }}/dns_records" {{ CFEMAIL }} {{ CFKEY }} '{{ DATA }}'
