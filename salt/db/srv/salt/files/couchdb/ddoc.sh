...
if [ "$REV" = 'null' ]; then
	/usr/bin/curl -s -u "$AUTH" -X PUT "$1"/_users/_design/User -d '@/home/couchdb/ddoc/User.json'
else
	DDOC=$(< /home/couchdb/ddoc/User.json sed "s/_design\/User\",/_design\/User\",\"_rev\": \"$REV\",/g")
